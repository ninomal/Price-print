...
class Ui():
    ...
    def tableEnter(self):
        ...
        labelPrinte = tk.Label(master=enterPane, text=f'Printer:\n {nameOfPrinter}',
                                background="#4A1985", font=("Helvetica", 20))
        labelPrinte.place(x= 5, y= 10)

        labelHours = tk.Label(master=enterPane, text='Hours/minuts:',
                                background="#4A1985", font=("Helvetica", 20))
        labelHours.place(x= 1, y= 110)
        entryHours = tk.Entry(master= enterPane, width= 15, font=("Helvetica", 18))
        entryHours.place(x=185, y =115)
    
        labelweight = tk.Label(master=enterPane, text='Weight grams:',
                                background="#4A1985", font=("Helvetica", 20))
        labelweight.place(x= 1, y= 180)
        entryWeight = tk.Entry(master= enterPane, width= 15, font=("Helvetica", 18))
        entryWeight.place(x=185, y =185)

        labelPrice = tk.Label(master=enterPane, text='You Price:',
                                background="#4A1985", font=("Helvetica", 20))
        labelPrice.place(x= 1, y= 250)
        entryPrice = tk.Entry(master= enterPane, width= 15, font=("Helvetica", 18))
        entryPrice.place(x=185, y =255)

        labelPrice = tk.Label(master=enterPane, text='Value:',
                                background="#4A1985", font=("Helvetica", 20))
        labelPrice.place(x= 1, y= 350)

        buttonEnter = tk.Button(master = enterPane, text="Calcule", font=("Helvetica", 18),
                               bg="#A580CA", command= self.tableAddUser)
        buttonEnter.place(x=249, y= 355, width=149)
